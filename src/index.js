...
app.get("/api/student/:id", (req, res) => {
    ...
    let flag = false
    try {
        for (let i = 0; i < studentsRecord.length; i++) if (req.params.id == studentsRecord[i].id) flag = studentsRecord[i]
        if (flag != false) {
            res.status(200).json({
                status: "ok",
                student:flag
...
app.post("/api/student", (req, res) => {
    try {
        ...
        if (name != "" && currentClass != "" && division != "") {
            studentsRecord.push({ id: Number(studentsRecord[studentsRecord.length - 1].id) + 1, ...req.body })
...
app.put("/api/student/:id", (req, res) => {
    try {
        res.header('Content-Type', 'application/x-www-form-urlencoded')
        let index = req.params.id, flag = false
        ...
        if (flag) {
            const { name, currentClass, division } = req.body
            if (name == "" || currentClass == "" || division == "")return res.status(400).json({
                status: "Not ok",
                message: "Student Details can't be empty"
            })
            else {
